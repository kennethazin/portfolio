...
import { Link, Box, Flex, Text, Stack, Icon } from '@chakra-ui/react';
...
const NavBar = props => {
  ...
  const toggle = () => {
    ...
    if (!isOpen) {
      // Only animate when opening the menu
      anime({
        targets: '.menuItem',
        translateX: [-50, 0],
        opacity: [0, 1],
...
const MenuToggle = ({ toggle, isOpen }) => {
  // toggles when the menu icon appears
  return (
...
const MenuItem = ({ children, isLast, to = '/portfolio/', icon, ...rest }) => {
  return (
    <Link as={NavLink} to={to} className="menuItem" {...rest}>
      {' '}
      {/* Use ChakraUI Link component */}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <MenuItem
          icon={FaGithub}
          to="https://github.com/kennethazin"
          target="_blank"
        >
          Source
        </MenuItem>
